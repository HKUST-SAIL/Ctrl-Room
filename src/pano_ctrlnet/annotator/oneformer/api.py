...
from annotator.oneformer.oneformer.demo.defaults import DefaultPredictor, DefaultPredictorHacked
...
def make_detectron2_model_hacked(config_path, ckpt_path):
    cfg = get_cfg()
    add_deeplab_config(cfg)
    add_common_config(cfg)
    add_swin_config(cfg)
    ...
    cfg.freeze()
    metadata = MetadataCatalog.get(cfg.DATASETS.TEST_PANOPTIC[0] if len(cfg.DATASETS.TEST_PANOPTIC) else "__unused")
    return DefaultPredictorHacked(cfg), metadata
...
def semantic_run_hacked(img, predictor, metadata):
    predictions = predictor(img[:, :, :, [2,1,0]], "semantic")  # Predictor of OneFormer must use BGR image !!!
    # print(f'predictions[sem_seg]: {predictions["sem_seg"].shape}')
    visualizer_map = Visualizer(img.squeeze(0).detach().cpu().numpy(), is_img=False, metadata=metadata, instance_mode=ColorMode.IMAGE)
    out_map = visualizer_map.draw_sem_seg(predictions["sem_seg"].argmax(dim=0).cpu(), alpha=1, is_text=False).get_image()
    return predictions["sem_seg"], out_map
