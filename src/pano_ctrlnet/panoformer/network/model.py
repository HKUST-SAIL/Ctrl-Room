...
class LeFF(nn.Module):
    ...
    def __init__(self, dim=32, hidden_dim=128, act_layer=nn.GELU, drop=0., flag=0):
        ...
        self.dwconv = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim, groups=hidden_dim, kernel_size=3, stride=1, padding=0), act_layer())
...
class Downsample(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, input_resolution=None):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_channel, out_channel, kernel_size=4, stride=2, padding=0),)
...
class InputProj(nn.Module):
    ...
    def __init__(self, in_channel=3, out_channel=64, kernel_size=3, stride=1, norm_layer=None, act_layer=nn.LeakyReLU):
        super().__init__()
        self.proj = nn.Sequential(nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=stride, padding=0),
                                  act_layer())
...
class OutputProj(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.input_resolution = input_resolution
        self.proj = nn.Sequential(nn.Conv2d(in_channel, out_channel, kernel_size=3, stride=stride, padding=1),)
...
class PanoformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.dattn = PanoSelfAttention(num_heads,
                                       dim,
                                       k=9,
                                       last_feat_height=self.input_resolution[0],
                                       last_feat_width=self.input_resolution[1],
                                       scales=1,
                                       dropout=0,
                                       need_attn=False)
    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            PanoformerBlock(dim=dim,
                            ...
                            token_mlp=token_mlp,
                            se_layer=se_layer,
                            ref_point=ref_point) for i in range(depth)
        ])
...
class Panoformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.input_proj = InputProj(in_channel=in_chans,
        ...
        # Decoder
        self.upsample_0 = upsample(embed_dim * 16,
                                   embed_dim * 8,
                                   input_resolution=(img_size // (2**4), img_size * 2 // (2**4)))
        self.decoderlayer_0 = BasicPanoformerLayer(dim=embed_dim * 16,
                                                   ...
                                                   ref_point=self.ref_point32x64,
                                                   flag=1)
        self.upsample_1 = upsample(embed_dim * 16,
                                   embed_dim * 4,
                                   input_resolution=(img_size // (2**3), img_size * 2 // (2**3)))
