...
class Demo:
    ...
    def plot_img_pairs(self, fig_size=3, alpha=0.45, scatter_size=10):
        ...
        def onclick(event):
            if event.inaxes == axes[0]:
                with torch.no_grad():
                    ...
                    src_ft = nn.Upsample(size=(512, 1024), mode='bilinear')(src_ft)
                    # print(f'upsampled src_feature shape: {src_ft.shape}')
                    ...
                    trg_ft = nn.Upsample(size=(512, 1024), mode='bilinear')(self.ft[1:])
                    # print(f'upsampled targ_feature shape: {trg_ft.shape}')

                    cos_map = cos(src_vec, trg_ft).cpu().numpy()  # N, H, W
                    # print(f'cos_similarity_map shape: {cos_map.shape}')
                    ...
                    for i in range(1, self.num_imgs):
                        ...
                        print(f'cos_similarity_map max value: {cos_map[i-1].max()}')
                        axes[i].clear()
