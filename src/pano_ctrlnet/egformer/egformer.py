...
import numpy as np
from src.pano_ctrlnet.egformer.Panoformer.model import *
...
def VRPE(
    num_heads, height, width, split_size
):  ## Used for vertical attention  / (theta,phi) -> (theta,phi') / This part is coded assuming that the split_size (stripe_with) is 1 to reduce the computational cost. To use larger split_size, ERPE must be calculated accoringly.
...
def HRPE(
    num_heads, height, width, split_size
):  ## Used for Horizontal attention  / (theta,phi) -> (theta',phi) / This part is coded assuming that the split_size (stripe_with) is 1 to reduce the computational cost. To use larger split_size, ERPE must be calculated accoringly.
    ...
    for k in range(0, height):
        base[k, :, :] = torch.sin(torch.as_tensor(k * pi / height)) * torch.sqrt(
            2 * (1 - torch.cos(base[k, :, :]))
        )  # height x W x W

    if (
        True
    ):  # Unlike the vertical direction, EIs are cyclic along the horizontal direction. Set to 'False' to reflect this cyclic characteristic / Refer to discussions in repo for more details.
        base = pn * base
...
class EGAttention(nn.Module):
    ...
    def im2hvwin(self, x):
        ...
        )  # B, H//H_sp, W//W_sp, H_sp * W_sp, C -> B, H//H_sp, W//W_sp, H_sp*W_sp, heads, C//heads
        return x
    ...
    def forward(self, qkv, res_x):
        ...
        if self.attention == "L":
            ...
            attn = 1 - torch.cos(
                attn
            )  # Square of the distance from the baseline point. By setting the baseline point as (1/sqrt(2),0,pi/2), DAS can get equal score range (0,1) for both vertical & horizontal direction.
...
class EGTransformer(nn.Module):
    ...
    def __init__(
            ...
            [
                (
                    ...
                    if i % 2 == 0
                        ...
                        dim=curr_dim,
                        num_heads=heads[0],
                        reso=[img_size[0] // 2, img_size[1] // 2],
                        mlp_ratio=mlp_ratio,
                        qkv_bias=qkv_bias,
                        qk_scale=qk_scale,
        ...
        self.downsample1 = Merge_Block(
            dim=curr_dim, dim_out=curr_dim * 2, resolution=[img_size[0] // 2, img_size[1] // 2]
        )
            ...
            [
                (
                    ...
                    if i % 2 == 0
                        ...
                        dim=curr_dim,
                        num_heads=heads[1],
                        reso=[img_size[0] // 4, img_size[1] // 4],
                        mlp_ratio=mlp_ratio,
                        qkv_bias=qkv_bias,
                        qk_scale=qk_scale,
        ...
        self.downsample2 = Merge_Block(
            dim=curr_dim, dim_out=curr_dim * 2, resolution=[img_size[0] // 4, img_size[1] // 4]
        )
        ...
        self.stage3 = nn.ModuleList(
            [
                EGBlock(
                    dim=curr_dim,
                    num_heads=heads[2],
                    reso=[img_size[0] // 8, img_size[1] // 8],
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
        ...
        self.downsample3 = Merge_Block(
            dim=curr_dim, dim_out=curr_dim * 2, resolution=[img_size[0] // 8, img_size[1] // 8]
        )
        ...
        self.stage4 = nn.ModuleList(
            [
                EGBlock(
                    dim=curr_dim,
                    num_heads=heads[3],
                    reso=[img_size[0] // 16, img_size[1] // 16],
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
        ...
        self.downsample4 = Merge_Block(
            dim=curr_dim, dim_out=curr_dim * 2, resolution=[img_size[0] // 16, img_size[1] // 16]
        )
        curr_dim = curr_dim * 2
        ...
        self.bottleneck = nn.ModuleList(
            [
                EGBlock(
                    dim=curr_dim,
                    num_heads=heads[-1],
                    reso=[img_size[0] // 32, img_size[1] // 32],
                    mlp_ratio=mlp_ratio,
                    qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
                    split_size=split_size[-1],
        ...
        self.upsample5 = Merge_Block(
            curr_dim, curr_dim // 2, resolution=[img_size[0] // 32, img_size[1] // 32], scale_factor=2.0
        )
        curr_dim = curr_dim // 2
        ...
        self.tune5 = Tune_Block(
            dim=curr_dim * 2, dim_out=curr_dim, resolution=[img_size[0] // 16, img_size[1] // 16]
        )  # Tune_5
        ...
        self.upsample6 = Merge_Block(
            dim=curr_dim, dim_out=curr_dim // 2, resolution=[img_size[0] // 16, img_size[1] // 16], scale_factor=2.0
        )
        curr_dim = curr_dim // 2
        self.dec_stage6 = nn.ModuleList(
            [
                EGBlock(
                    ...
                    qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
                    split_size=split_size[5],
                    drop=drop_rate,
                    attn_drop=attn_drop_rate,
                    drop_path=dpr[np.sum(depth[:5]) + i],
                    norm_layer=norm_layer,
                    attention=0,
                    idx=i % 2,
                    depth_index=2,
        ...
        self.upsample7 = Merge_Block(
            curr_dim, curr_dim // 2, resolution=[img_size[0] // 8, img_size[1] // 8], scale_factor=2.0
        )
        curr_dim = curr_dim // 2
            ...
            [
                (
                    ...
                    if i % 2 == 0
                        ...
                        qkv_bias=qkv_bias,
                        qk_scale=qk_scale,
                        split_size=split_size[6],
                        drop=drop_rate,
                        attn_drop=attn_drop_rate,
                        drop_path=dpr[np.sum(depth[:6]) + i],
                        norm_layer=norm_layer,
                        attention=0,
                        idx=i % 2,
                        depth_index=1,
        ...
        self.upsample8 = Merge_Block(
            curr_dim, curr_dim // 2, resolution=[img_size[0] // 4, img_size[1] // 4], scale_factor=2.0
        )
        curr_dim = curr_dim // 2
            ...
            [
                (
                    ...
                    if i % 2 == 0
                        ...
                        drop=drop_rate,
                        attn_drop=attn_drop_rate,
                        drop_path=dpr[np.sum(depth[:7]) + i],
                        norm_layer=norm_layer,
                        attention=0,
                        idx=i % 2,
                        depth_index=0,
    ...
    def forward_features(self, x):
        ...
        # print(f'EGTransformer:forward_features: patch_embed x.shape {x.shape}')
        ...
        # print(f'EGTransformer:forward_features: downsample1 x.shape {x.shape}')
        ...
        # print(f'EGTransformer:forward_features: downsample2 x.shape {x.shape}')
        ...
        # print(f'EGTransformer:forward_features: downsample3 x.shape {x.shape}')
        ...
        # print(f'EGTransformer:forward_features: downsample4 x.shape {x.shape}')

        for blk in self.bottleneck:
            x = blk(x)
        # print(f'EGTransformer:forward_features: bottleneck x.shape {x.shape}')
        ...
        # print(f'EGTransformer:forward_features: dec_stage5 before x.shape {x.shape}')
        x = self.tune5(x)
        x = self.rearrange[0](x)
        # print(f'EGTransformer:forward_features: dec_stage5 x.shape {x.shape}')

        x = self.upsample6(x)
        for blk in self.dec_stage6:
            x = blk(x)
        x = torch.cat((self.set_dim[1](features[2]), self.set_dim[1](x)), dim=1)
        # print(f'EGTransformer:forward_features: dec_stage6 before x.shape {x.shape}')
        x = self.tune6(x)
        x = self.rearrange[1](x)
        # print(f'EGTransformer:forward_features: dec_stage6 x.shape {x.shape}')

        x = self.upsample7(x)
        for blk in self.dec_stage7:
            x = blk(x)
        x = torch.cat((self.set_dim[2](features[1]), self.set_dim[2](x)), dim=1)
        # print(f'EGTransformer:forward_features: dec_stage7 before x.shape {x.shape}')
        x = self.tune7(x)
        x = self.rearrange[2](x)
        # print(f'EGTransformer:forward_features: dec_stage7 x.shape {x.shape}')

        x = self.upsample8(x)
        for blk in self.dec_stage8:
            x = blk(x)
        x = torch.cat((self.set_dim[3](features[0]), self.set_dim[3](x)), dim=1)
        # print(f'EGTransformer:forward_features: dec_stage8 before x.shape {x.shape}')
        x = self.tune8(x)
        x = self.rearrange[3](x)
        # print(f'EGTransformer:forward_features: dec_stage8 x.shape {x.shape}')

        # EGformer Output Projection
        x = self.tune_final(x)
        # print(f'EGTransformer:forward_features: tune_final x.shape {x.shape}')
        x = self.output_conv(x)
        # print(f'EGTransformer:forward_features: output_conv x.shape {x.shape}')

        return x

    def forward(self, x):
        # print(f'EGTransformer:forward: x.shape {x.shape}')
        out = self.forward_features(x)
        return out


class EGDepthModel(nn.Module):
    def __init__(self, **kwargs) -> None:
        super().__init__()
        split_size = [1, 1, 1, 1, 1, 1, 1, 1, 1]
        img_size = [512, 1024]
        num_heads = [1, 2, 4, 8, 8, 4, 2, 1, 16]
        self.module = EGTransformer(
            patch_size=4,
            embed_dim=32,
